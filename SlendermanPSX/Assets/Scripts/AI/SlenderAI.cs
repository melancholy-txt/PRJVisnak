    ...
    void Update()
    {
        ...
        if (GeometryUtility.TestPlanesAABB(planes, slenderMesh.bounds) && playerHealth > 0)
        {
            ...
            if (raycastScript.detected == true)
            {
                ...
                staticVolume += soundIncreaseRate * Time.deltaTime;  //staticVolume will increase by the amount determined by the soundIncreaseRate
                staticAmount += staticIncreaseRate * Time.deltaTime; //staticAmount will increase by the amount determined by the staticIncreaseRate
                playerHealth -= healthDecreaseRate * Time.deltaTime; //playerHealth will decrease by the amount determined by healthDecreaseRate
        ...
        if (!GeometryUtility.TestPlanesAABB(planes, slenderMesh.bounds) && playerHealth > 0 || raycastScript.detected == false && playerHealth > 0)
        {
            ...
            staticAmount -= staticDecreaseRate * Time.deltaTime; //staticAmount will decrease by the amount determined by the staticDecreaseRate
            staticVolume -= soundDecreaseRate * Time.deltaTime; //staticVolume will decrease by the amount determined by the soundDecreaseRate
            playerHealth += healthIncreaseRate * Time.deltaTime; //playerHealth will increase by the amount determined by healthIncreaseRate
